...
typeToMode(int type) {
  ...
  int mode = type - XD3_CPY;
  return mode;
}

static char*
typeToTypeString(int type) {
  if (type == XD3_NOOP) {
    return "XD3_NOOP";
  }
  if (type == XD3_ADD) {
...
static void
dumpCodeTable(xd3_dinst *tbl, int startRow, int endRow) {
  xd3_dinst *d = tbl;
  d += startRow;
  printf("==============================================\n");
  printf("code table:\n");
  for (int i = startRow; i < endRow; i++, d++) {
    printf("%3d: %s(%d), %2d, %d, %s(%d), %2d, %d\n", i,
           typeToTypeString(d->type1), d->type1, d->size1, typeToMode(d->type1),
           typeToTypeString(d->type2), d->type2, d->size2, typeToMode(d->type2));
  }
  printf("==============================================\n");
}
...
xd3_build_code_table (const xd3_code_table_desc *desc, xd3_dinst *tbl)
{
  ...
  dumpCodeTable(tbl, startRow, row);
  startRow = row;
...
{
  printf("adler32: adler = %d\n", adler);
  printf("adler32: len = %d\n", len);
...
{
  printf("acache->s_near = %d\n", acache->s_near);
  if (acache->s_near > 0)
    {
      acache->near_array[acache->next_slot] = addr;
      printf("acache->near_array[%d] = %d\n", acache->next_slot, addr);
      acache->next_slot = (acache->next_slot + 1) % acache->s_near;
      printf("acache->next_slot = %d\n", acache->next_slot);
    }

  printf("acache->s_same = %d\n", acache->s_same);
  if (acache->s_same > 0)
    {
      acache->same_array[addr % (acache->s_same*256)] = addr;
      printf("acache->same_array[%d] = %d\n", addr % (acache->s_same*256), addr);
    }
}
		    ...
		    const uint8_t *max, usize_t *valp)
{
  ...
  if (mode < same_start)
	...
	case VCD_HERE: // 1
	  ...
	  break;
	default:
          printf("add near_array[%d] = %d\n", mode-2, stream->acache.near_array[mode - 2]);
...
xd3_encode_init (xd3_stream *stream, int full_init)
{
  ...
  printf("set stream->acache.s_near = %d\n", stream->acache.s_near);
...
xd3_decode_stream (xd3_stream    *stream,
		   ...
		   usize_t        output_size_max)
{
  printf("xd3_decode_stream: file/line = %s/%d\n", __FILE__, __LINE__);
...
xd3_decode_memory (const uint8_t *input,
		   ...
		   usize_t        output_size_max,
		   int            flags) {
  printf("xd3_decode_memory: file/line = %s/%d\n", __FILE__, __LINE__);
